...
def compile_to_file(table, header):
    csv_path = './'+year+'.csv'
    with open(csv_path,'a+') as f:
        csv_write = csv.writer(f)
    table.to_csv(csv_path,mode = 'a',index = False,header = header)

def main():
    logging_setup()
    print('logging setup done')
    lg.debug('logging setup done')
    #download zip files
    downloading()
    ...
    for filename in filename_list:
        ...
        if os.path.isfile(filepath):
        	...
        	lg.debug(filepath+' does not exist in the file system')
        	continue
        ...
        # not finished
        print('drop column date and time')
        lg.debug('drop column date and time')
        table_copy = table_copy.drop(columns= ['date','time'])
        # save the table into a new csv file
        # not finished
        print('compile 12 files to 1 file')
        lg.debug('compile 12 files to 1 file')
        if filename == filename_list[0]:
            compile_to_file(table_copy, True)
        else:
